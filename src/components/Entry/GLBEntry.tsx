...
interface GLBEntryProps {
  onComplete: () => void
}

// GLB Chest Component with Error Handling
function ChestModel({ isUnlocked }: { isUnlocked: boolean }) {
  ...
  // Fallback to procedural chest if GLB fails
  if (loadError || !gltf) {
    return <ProceduralChest isUnlocked={isUnlocked} />
  }
...
export default function GLBEntry({ onComplete }: GLBEntryProps) {
    ...
    >
      ...
      >
        <Suspense fallback={null}>
          ...
          {/* Chest Model */}
          <ChestModel isUnlocked={isUnlocked} />
          ...
          >
            ...
            >
               <div className="text-[#D4AF37] text-lg tracking-[0.3em] mb-4 uppercase font-light">
                 {hasError ? 'Using Fallback Mode' : 'Preparing Your Experience'}
               </div>
               <div className="w-12 h-12 border-2 border-[#D4AF37] border-t-transparent rounded-full animate-spin mx-auto" />
               {hasError && (
                 <div className="text-[#D4AF37]/70 text-sm mt-4">
                   GLB model unavailable - showing procedural chest
                 </div>
               )}
            </motion.div>
          </motion.div>
