...
// GLB Chest with fallback
function GLBChestWithFallback({ isUnlocked, onUnlockComplete }: { isUnlocked: boolean, onUnlockComplete: () => void }) {
  const [useGLB, setUseGLB] = useState(true)
  const [glbError, setGlbError] = useState(false)
  
  // Try to load GLB, fallback to procedural if it fails
  let gltf
  try {
    if (useGLB && !glbError) {
  ...
  // If GLB loaded successfully, use it
  if (gltf && !glbError) {
    const chestRef = useRef<THREE.Group>(null)
    const { scene } = gltf
    const { camera } = useThree()
  ...
  // Fallback to procedural chest
  return <ProceduralChest isUnlocked={isUnlocked} onUnlockComplete={onUnlockComplete} />
}
...
// Main Scene
export default function WorkingChestScene({ isUnlocked, onUnlockComplete }: ChestSceneProps) {
  return (
    <>
      {/* Lighting */}
      ...
      {/* Chest with GLB fallback */}
      <Suspense fallback={<ProceduralChest isUnlocked={isUnlocked} onUnlockComplete={onUnlockComplete} />}>
        <GLBChestWithFallback 
